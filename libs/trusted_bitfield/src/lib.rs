...
/// Model of the actual AtomicU64 type used for Prusti verification.
#[cfg(prusti)]
impl AtomicU64 {
    #[pure]
    #[trusted]
...
/// A linear type for a verified allocation address. `first_fit_and_set` is the only function that can create a `VerifiedAllocAddr`.
pub struct VerifiedAllocAddr(usize);

/// Unwrap a `VerifiedAllocAddr` to get the underlying `usize` address.
pub fn addr_from_verified_alloc_addr(addr: VerifiedAllocAddr) -> usize {
    addr.0
}

/// A linear type for a verified deallocation address. The only exposed function capable of creating this type is generated by
/// `dealloc_callback`.
...
/// Returns `2^k`. `k` must be strictly less than 64.
#[allow(dead_code)]
#[pure]
#[trusted]
#[requires(k < 64)]
#[ensures(k == 0 ==> result == 1)]  // Base case described in Verus.
...
/// Check that a bit in a `u64` is set.
#[pure]
#[trusted]
#[requires(idx < 64)]
...
/// Make a `u64` with `k` trailing zeroes and `64 - k` leading ones.
#[pure]
#[trusted]
#[requires(k <= 64)]
...
/// How many ones before the first zero.
#[trusted]
...
fn my_trailing_ones(u: &u64) -> usize {
    ...
    // A "safe" version of the above, which Prusti can verify.
    //
    // let mut k = 0;
    // while k < 64 {
    //     body_invariant!(k < 64);
...
/// How many zeroes before the first one.
#[trusted]
#[ensures(result <= 64)]  // The result should be within the range of the u64.
...
/// Initialize a bitfield, given a relevant number of bits.
...
/// Check that the bit at `idx` is set in the bitfield.
#[pure]
#[requires(bitfield.len() > 0)]
...
/// Check that all relevant bits are set to 1 in the bitfield.
#[requires(bitfield.len() > 0)]
...
/// Check that all relevant bits are set to 0 in the bitfield.
#[requires(bitfield.len() > 0)]
#[requires(relevant_bits <= bitfield.len() * 64)]
#[requires(relevant_bits > 0)]
...
/// Clear a bit at index `idx` in the bitfield.
...
/// Set a bit at index `idx` in the bitfield.
#[ensures(idx < bitfield.len() * 64 ==> {  // Check that the bitfield isn't changed if the bit
...
/// Find the first free bit in a `u64`.
#[requires(u < u64::MAX)]  // The input should always have at least one free bit.
...
/// Find the first free bit in a `u64`, and check whether it satisfies the layout requirements.
...
/// Find the first free bit in a bitfield, and check whether it satisfies the layout requirements.
...
impl TrustedBitfield8 {
    ...
    /// Create and initialize the bitfield
    ///
    /// # Arguments
    ...
    /// Tries to find a free block of memory that satisfies `alignment` requirement. Once found, we set it to "allocated" immediately to
    /// prevent potentially incorrect behavior. Returns a value of linear type `VerifiedAllocAddr`.
    ...
    /// Clears the bit at the given verified address. Returns `true` if successfully cleared.
    ...
    /// Returns a callback that can be used to generate a VerifiedDeallocAddr to deallocate a given address.
    #[trusted]
